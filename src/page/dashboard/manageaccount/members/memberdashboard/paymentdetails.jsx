...
const MemberPaymentdetails = ({username}) => {
  ...
  useEffect(() => {
    fetch(`${process.env.REACT_APP_API_URL}members/getmemberpaydetail`,{
      method: "POST",
      credentials: 'include',
      headers: {
      ...
      if(country.value.toUpperCase() !== "PH" && detail.address.Country !== "PH"){
        Swal.fire({
          ...
          if (result.isConfirmed) {
            fetch(`${process.env.REACT_APP_API_URL}members/updatepaymentdetail`,{
              method: "POST",
              credentials: 'include',
              headers: {
                "Content-Type": "application/json"
