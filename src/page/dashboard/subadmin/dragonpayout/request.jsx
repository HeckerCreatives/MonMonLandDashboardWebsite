...
const SubAdminPayoutDragonRequest = () => {
    ...
    const handleRequest = (id) => {
        ...
        Swal.fire({
            icon: "warning",
            title: "Are you sure you want to process this payout?",
            ...
            if(ok.isConfirmed){
                setIsLoading(false)
                fetch(`${process.env.REACT_APP_API_URL}dragonpayout/process/${id}`, {
                    method: "POST",
                    credentials: 'include',
                    headers: {
                        "Content-Type": "application/json",
    ...
    const handleReject = (id) => {
        ...
        Swal.fire({
            ...
            if(ok.isConfirmed){
                setIsLoading(false)
                fetch(`${process.env.REACT_APP_API_URL}dragonpayout/reject/${id}`, {
                    method: "POST",
                    credentials: 'include',
                    headers: {
                        "Content-Type": "application/json",
            ...
            <MDBTable align="middle" responsive className="mt-3 text-center">
                ...
                { filteredRequest.length !== 0 ?
                    ...
                    return(
                        ...
                        <td>
                            <MDBBtn className="m-1" onClick={() => handleRequest(data._id)}>
                            {isloading ? <MDBSpinner  size="sm" role='status' grow/> : "Process"}
                            </MDBBtn>
                            <MDBBtn className="m-1" onClick={() => handleReject(data._id)}>
...
export default SubAdminPayoutDragonRequest;